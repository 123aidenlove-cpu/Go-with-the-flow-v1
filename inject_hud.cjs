...
const hudReplacement = `      </TransformWrapper>
      ...
      <div className="absolute top-6 right-6 flex flex-col gap-4 z-50 items-end">
        ...
        >
          ...
          <span className="font-bold text-lg hidden sm:block tracking-wide text-white">Change Profile</span>
        </button>
...
const replaceRegex = /<\/TransformWrapper>[\s\S]*$/;
content = content.replace(replaceRegex, hudReplacement);

// Also make sure to add Bell to lucide-react if missing
if (!content.includes('Bell')) {
  content = content.replace(/import \{ User \} from 'lucide-react';/, "import { User, Bell } from 'lucide-react';");
}
if (!content.includes('getXP')) {
  content = content.replace(/import \{ TransformWrapper/, "import { getXP, getQuavits, getRareQuavits } from '../utils/economy';\nimport { TransformWrapper");
}
