...
export const CelloFingeringChart: React.FC<FingeringChartProps> = ({ fingeringString = "", interactive = false, showNoteNames = false, onChange }) => {
  ...
  tokens.forEach(token => {
    const match = token.match(/([cgda])(\d[\+\-]?)/i);
    if (match) {
      active.add(`${match[1].toUpperCase()}${match[2]}`);
    }
  });

  const strings = ['C', 'G', 'D', 'A']; // Left to right
  
  // Array of position objects for Cello
  const positions = [
    { id: '1-', label: 'Low 1', isMain: false, semitones: 1 },
    { id: '1', label: '1', isMain: true, semitones: 2 },
    { id: '2', label: '2', isMain: false, semitones: 3 },
    { id: '3', label: '3', isMain: true, semitones: 4 },
    { id: '4', label: '4', isMain: true, semitones: 5 },
    { id: '4+', label: 'High 4 / Ext', isMain: false, semitones: 6 }
  ];

  const noteMap: Record<string, Record<string, string>> = {
    'C': { '0': 'C', '1-': 'C#', '1': 'D', '2': 'D#', '3': 'E', '4': 'F', '4+': 'F#' },
    'G': { '0': 'G', '1-': 'G#', '1': 'A', '2': 'Bb', '3': 'B', '4': 'C', '4+': 'C#' },
    'D': { '0': 'D', '1-': 'Eb', '1': 'E', '2': 'F', '3': 'F#', '4': 'G', '4+': 'G#' },
    'A': { '0': 'A', '1-': 'Bb', '1': 'B', '2': 'C', '3': 'C#', '4': 'D', '4+': 'D#' }
  };
      ...
      <div className={`relative w-full flex flex-col pt-4 pb-8 ${showNoteNames ? 'pl-12 pr-28' : 'pl-8 pr-24'}`}>
        ...
        <div className="relative w-full h-[320px] mt-4 z-20">
          ...
          <div className="absolute inset-0 z-10 pointer-events-none">
            {strings.map((str, idx) => (
              <div key={str} className="absolute w-[3px] h-full bg-slate-400 shadow-sm -ml-[1px]" style={{ left: `${idx * 33.333}%` }} />
            ))}
          </div>
                ...
                return (
                  <div key={str} className={`absolute w-12 h-12 flex items-center justify-center -ml-6 -mt-6 z-40 ${interactive ? 'cursor-pointer' : ''}`} style={{ left: `${idx * 33.333}%` }} onClick={() => togglePosition(id)}>
                    {(isFilled || interactive) && (
                      <div className={`rounded-full border-2 transition-all duration-300 flex items-center justify-center ${showNoteNames ? 'w-7 h-7' : 'w-6 h-6'} ${isFilled ? `bg-amber-600 border-amber-600 shadow-md ${showNoteNames ? 'scale-110' : ''}` : 'bg-white border-slate-300 opacity-0 group-hover:opacity-50'}`}>
          ...
          {/* Positions */}
          {positions.map((pos) => {
            const visualPct = ((1 - Math.pow(2, -pos.semitones / 12)) / (1 - Math.pow(2, -8/12))) * 100;
            return (
                  ...
                  return (
                    <div key={str} className="absolute w-8 flex justify-center -ml-4" style={{ left: `${idx * 33.333}%` }}>
                      <div 
                        ...
                        className={`rounded-full border-2 transition-all duration-300 z-30 flex items-center justify-center -mt-3 ${showNoteNames ? 'w-7 h-7' : 'w-6 h-6'} ${isFilled ? `bg-amber-600 border-amber-600 shadow-md ${showNoteNames ? 'scale-110' : ''}` : 'bg-white border-slate-300 opacity-0 group-hover:opacity-50'} ${interactive ? 'cursor-pointer' : ''}`}>
